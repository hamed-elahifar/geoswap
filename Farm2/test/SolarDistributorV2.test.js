...
describe("GeosDistributorV2", function () {
  before(async function () {
    ...
    this.GeosDistributorV2 = await ethers.getContractFactory(
      "GeosDistributorV2"
    );
    this.GeosSwapTokenV2 = await ethers.getContractFactory("MockERC20");
    this.ERC20Mock = await ethers.getContractFactory("MockERC20", this.minter);
    // this.ComplexRewarderPerSec = await ethers.getContractFactory(
    ...
    this.secOffset = 1;
    this.tokenOffset = 1;
    this.reward = (sec, percent) => (sec * this.geosPerSec * percent) / 1000;
  ...
  beforeEach(async function () {
    this.geos = await this.GeosSwapTokenV2.deploy("Geos Token", "GEOS", "0");
    await this.geos.deployed();
  ...
  context("constructor", function () {
    it("should revert contract creation if team percent don't meet criteria", async function () {
      ...
      ).to.be.revertedWith("constructor: invalid team percent value");
    });
    ...
    it("should set internal variables after deployed", async function () {
      const gdv2 = await this.GeosDistributorV2.deploy(
        this.geos.address,
        this.geosPerSec,
        this.dev.address,
        this.treasury.address,
      ...
      const startTimestamp = await gdv2.startTimestamp();
      const geos = await gdv2.geos();
      const geosPerSec = await gdv2.geosPerSec();

      const teamAddress = await gdv2.teamAddress();
      const treasuryAddress = await gdv2.treasuryAddress();
      const investorAddress = await gdv2.investorAddress();

      const teamPercent = await gdv2.teamPercent();
      const treasuryPercent = await gdv2.treasuryPercent();
      const investorPercent = await gdv2.investorPercent();

      const startTime = (await latest()).add(60 * 60 * 24 * 365);

      expect(geos).to.equal(this.geos.address);
      expect(geosPerSec).to.equal(this.geosPerSec);
  ...
  context("setters", function () {
    it("should allow team update himself", async function () {
      ...
      await expect(
        gdv2
          .connect(this.bob)
          .setTeamAddress(this.bob.address, { from: this.bob.address })
      ).to.be.revertedWith(
        "set team address: only previous team address can call this method"
      );
      await gdv2
        .connect(this.dev)
        .setTeamAddress(this.bob.address, { from: this.dev.address });
      expect(await gdv2.teamAddress()).to.equal(this.bob.address);
    });
    ...
    it("should allow treasury update himself", async function () {
      ...
      await expect(
        gdv2
          .connect(this.bob)
          .setTreasuryAddress(this.bob.address, { from: this.bob.address })
      ).to.be.revertedWith(
        "set treasury address: only previous treasury address can call this method"
      );
      await gdv2
        .connect(this.treasury)
        .setTreasuryAddress(this.bob.address, { from: this.treasury.address });
      expect(await gdv2.treasuryAddress()).to.equal(this.bob.address);
    });
    ...
    it("should allow investor update himself", async function () {
      ...
      await expect(
        gdv2
          .connect(this.bob)
          .setInvestorAddress(this.bob.address, { from: this.bob.address })
      ).to.be.revertedWith(
        "set investor address: only previous investor can call this method"
      );
      await gdv2
        .connect(this.investor)
        .setInvestorAddress(this.bob.address, { from: this.investor.address });
      expect(await gdv2.investorAddress()).to.equal(this.bob.address);
    });
  });
  ...
  context("distributor", function () {
    ...
    it("should distribute GEOS properly for each staker", async function () {
      this.gdv2 = await this.GeosDistributorV2.deploy(
        this.geos.address,
        this.geosPerSec,
        this.dev.address,
        this.treasury.address,
      ...
      await this.gdv2.deployed();

      await this.gdv2.add("100", this.lp.address, 0, 0);

      await this.lp.connect(this.alice).approve(this.gdv2.address, "1000", {
        from: this.alice.address,
      });
      await this.lp.connect(this.bob).approve(this.gdv2.address, "1000", {
        from: this.bob.address,
      });
      await this.lp.connect(this.carol).approve(this.gdv2.address, "1000", {
        from: this.carol.address,
      });

      await this.gdv2.startFarming();

      // Alice deposits 10 LPs at t+10
      await advanceTimeAndBlock(1); // t+9, b=22
      await this.gdv2
        .connect(this.alice)
        .deposit(0, "10", { from: this.alice.address }); // t+10, b=23
      // Bob deposits 20 LPs at t+14
      await advanceTimeAndBlock(3); // t+13, b=24
      await this.gdv2.connect(this.bob).deposit(0, "20"); // t+14, b=25
      // Carol deposits 30 LPs at block t+18
      await advanceTimeAndBlock(3); // t+17, b=26
      await this.gdv2
        .connect(this.carol)
        .deposit(0, "30", { from: this.carol.address }); // t+18, b=27

      // Alice deposits 10 more LPs at t+20. At this point:
      //   Alice should have:
      //      - 4*50 + 4*50*1/3 + 2*50*1/6 = 283 (+50) GeosSwapTokenV2
      ...
      await this.gdv2
        .connect(this.alice)
        .deposit(0, "10", { from: this.alice.address }); // t+20, b=29
      expect(await this.geos.totalSupply()).to.be.within(1000, 1100);

      // Because LP rewards are divided among participants and rounded down, we account
      // for rounding errors with an offset
      expect(await this.geos.balanceOf(this.alice.address)).to.be.within(
        283 - this.tokenOffset,
        333 + this.tokenOffset
      );

      console.log(
        "Balance of Alice at t+20",
        await this.geos.balanceOf(this.alice.address)
      );

      expect(await this.geos.balanceOf(this.bob.address)).to.equal("0");

      console.log(
        "Balance of Bob at t+20",
        await this.geos.balanceOf(this.bob.address)
      );

      expect(await this.geos.balanceOf(this.carol.address)).to.equal("0");

      console.log(
        "Balance of Carol at t+20",
        await this.geos.balanceOf(this.carol.address)
      );

      console.log(
        "Balance of Dev at t+20",
        await this.geos.balanceOf(this.dev.address)
      );

      expect(await this.geos.balanceOf(this.dev.address)).to.be.within(
        200 - this.tokenOffset,
        220 + this.tokenOffset
      );

      console.log(
        "Balance of Treasury at t+20",
        await this.geos.balanceOf(this.treasury.address)
      );

      expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(
        200 - this.tokenOffset,
        220 + this.tokenOffset
      );

      console.log(
        "Balance of Investor at t+20",
        await this.geos.balanceOf(this.investor.address)
      );

      expect(await this.geos.balanceOf(this.investor.address)).to.be.within(
        100 - this.tokenOffset,
        110 + this.tokenOffset
      );

      console.log(
        "Balance of MasterChef at t+20",
        await this.geos.balanceOf(this.gdv2.address)
      );

      expect(await this.geos.balanceOf(this.gdv2.address)).to.be.within(
        217 - this.tokenOffset,
        317 + this.tokenOffset
      );

      // Bob withdraws 5 LPs at t+30. At this point:
      //   Bob should have:
      //     - 4*50*2/3 + 2*50*2/6 + 10*50*2/7 = 309 (+50) GeosSwapTokenV2
      //   Dev should have: 20*20= 400 (+20)
      //   Treasury should have: 20*20 = 400 (+20)
      //   Investor should have: 20*10 = 200 (+10)
      //   gdv2 should have: 217 + 1000 - 309 - 200 - 200 - 100 = 408 (+100)
      await advanceTimeAndBlock(9); // t+29, b=32
      await this.gdv2
        .connect(this.bob)
        .withdraw(0, "5", { from: this.bob.address }); // t+30, b=33
      expect(await this.geos.totalSupply()).to.be.within(2000, 2100);
      ...
      expect(await this.geos.balanceOf(this.bob.address)).to.be.within(
        309 - this.tokenOffset,
        359 + this.tokenOffset
      );

      expect(await this.geos.balanceOf(this.carol.address)).to.equal("0");

      expect(await this.geos.balanceOf(this.dev.address)).to.be.within(
        400 - this.tokenOffset,
        420 + this.tokenOffset
      );
      expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(
        400 - this.tokenOffset,
        420 + this.tokenOffset
      ...
      expect(await this.geos.balanceOf(this.investor.address)).to.be.within(
        200 - this.tokenOffset,
        210 + this.tokenOffset
      );
      expect(await this.geos.balanceOf(this.gdv2.address)).to.be.within(
        408 - this.tokenOffset,
        508 + this.tokenOffset
      );
      ...
      await this.gdv2
        .connect(this.alice)
        .withdraw(0, "20", { from: this.alice.address }); // t+40, b=35
      await advanceTimeAndBlock(9); // t+49, b=36
      await this.gdv2
        .connect(this.bob)
        .withdraw(0, "15", { from: this.bob.address }); // t+50, b=37
      await advanceTimeAndBlock(9); // t+59, b=38
      await this.gdv2
        .connect(this.carol)
        .withdraw(0, "30", { from: this.carol.address }); // t+60, b=39
      expect(await this.geos.totalSupply()).to.be.within(5000, 5100);
      // Alice should have:
      //  - 283 + 10*50*2/7 + 10*50*20/65 = 579 (+50) GeosSwapTokenV2
      //  - 120 + 2*40*2/7 + 2*40*20/65 = 167 PartnerToken
      expect(await this.geos.balanceOf(this.alice.address)).to.be.within(
        579 - this.tokenOffset,
        629 + this.tokenOffset
      );

      // Bob should have:
      //  - 309 + 10*50*15/65 + 10*50*15/45 = 591 (+50) GeosSwapTokenV2
      //  - 102 + 2*40*15/65 + 2*40*15/45 = 147 PartnerToken
      expect(await this.geos.balanceOf(this.bob.address)).to.be.within(
        591 - this.tokenOffset,
        641 + this.tokenOffset
      );

      // Carol should have:
      //  - 2*50*3/6 + 10*50*3/7 + 10*50*30/65 + 10*50*30/45 + 10*50 = 1445 (+50) GeosSwapTokenV2
      //  - 2*40*1/2 + 2*40*3/7 + 2*40*30/65 + 2*40*30/45 + 2*40 = 244 PartnerToken
      expect(await this.geos.balanceOf(this.carol.address)).to.be.within(
        1328 - this.tokenOffset,
        1378 + this.tokenOffset
      );
      ...
      expect(await this.geos.balanceOf(this.dev.address)).to.be.within(
        1000 - this.tokenOffset,
        1020 + this.tokenOffset
      );
      expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(
        1000 - this.tokenOffset,
        1020 + this.tokenOffset
      ...
      expect(await this.geos.balanceOf(this.investor.address)).to.be.within(
        500 - this.tokenOffset,
        510 + this.tokenOffset
      );
      // gdv2 should have nothing
      expect(await this.geos.balanceOf(this.gdv2.address)).to.be.within(
        0,
        0 + this.tokenOffset
      );
    ...
    it("should give proper GEOS allocation to each pool", async function () {
      this.gdv2 = await this.GeosDistributorV2.deploy(
        this.geos.address,
        this.geosPerSec,
        this.dev.address,
      ...
      await this.gdv2.deployed(); // t-59, b=14

      await this.lp
        .connect(this.alice)
        .approve(this.gdv2.address, "1000", { from: this.alice.address }); // t-55, b=18
      await this.lp2
        .connect(this.bob)
        .approve(this.gdv2.address, "1000", { from: this.bob.address }); // t-54, b=19
      // Add first LP to the pool with allocation 10
      await this.gdv2.add("10", this.lp.address, 0, 0);
      ...
      await this.gdv2
        .connect(this.alice)
        .deposit(0, "10", { from: this.alice.address }); // t+10, b=22
      // Add LP2 to the pool with allocation 20 at t+20
      
      await advanceTimeAndBlock(9); // t+19, b=23
      await this.gdv2.add("20", this.lp2.address, 0, 0);
      // Alice's pending reward should be:
      //   - 10*50 = 500 (+50) GeosSwapTokenV2

      expect(
        (await this.gdv2.pendingTokens(0, this.alice.address)).amounts[0]
      ).to.be.within(500 - this.tokenOffset, 550 + this.tokenOffset);

      await advanceTimeAndBlock(4); // t+24, b=25
      await this.gdv2
        .connect(this.bob)
        .deposit(1, "10", { from: this.bob.address }); // t+25, b=26
      // Alice's pending reward should be:
      //   - 500 + 5*1/3*50 = 583 (+50) GeosSwapTokenV2
      expect(
        (await this.gdv2.pendingTokens(0, this.alice.address)).amounts[0]
      ).to.be.within(583 - this.tokenOffset, 633 + this.tokenOffset);

      // At this point:
      //   Alice's pending reward should be:
      //     - 583 + 5*1/3*50 = 666 (+50) GeosSwapTokenV2
      // Bob's pending reward should be:
      //     - 5*2/3*50 = 166 (+50) GeosSwapTokenV2
      await advanceTimeAndBlock(5); // t+30, b=27
      expect(
        (await this.gdv2.pendingTokens(0, this.alice.address)).amounts[0]
      ).to.be.within(666 - this.tokenOffset, 716 + this.tokenOffset);
      expect(
        (await this.gdv2.pendingTokens(1, this.bob.address)).amounts[0]
      ).to.be.within(166 - this.tokenOffset, 216 + this.tokenOffset);
      expect(
        (await this.gdv2.pendingTokens(1, this.alice.address)).amounts[0]
      ).to.equal("0");
      expect(
        (await this.gdv2.pendingTokens(0, this.bob.address)).amounts[0]
      ).to.equal("0");

      // Make sure they have receive the same amount as what was pending
      await this.gdv2
        .connect(this.alice)
        .withdraw(0, "10", { from: this.alice.address }); // t+31, b=28
      // Alice should have:
      //   - 666 + 1*1/3*50 = 682 (+50) GeosSwapTokenV2
      expect(await this.geos.balanceOf(this.alice.address)).to.be.within(
        682 - this.tokenOffset,
        732 + this.tokenOffset
      );

      await this.gdv2
        .connect(this.bob)
        .withdraw(1, "5", { from: this.bob.address }); // t+32, b=29
      // Bob should have:
      //   - 166 + 2*2/3*50 = 232 (+50) GeosSwapTokenV2
      expect(await this.geos.balanceOf(this.bob.address)).to.be.within(
        232 - this.tokenOffset,
        282 + this.tokenOffset
      );
    });
  });

  xcontext("rewarder", function () {
    beforeEach(async function () {
      this.partnerToken = await this.ERC20Mock.deploy(
        "Reward",
        "RWD",
        "10000000000"
      );
    ...
    //     this.gdv2 = await this.GeosDistributorV2.deploy(this.geos.address, this.geosPerSec, this.dev.address, this.treasury.address, this.investor.address, this.devPercent, this.treasuryPercent, this.investorPercent);
    //     await this.gdv2.deployed();
    //     await this.gdv2.startFarming();

    //     this.rewarder = await this.ComplexRewarderPerSec.deploy(this.partnerToken.address, this.gdv2.address, false);
    //     await this.rewarder.deployed();
    //     await this.partnerToken.connect(this.deployer).approve(this.rewarder.address, "1000");

    //     await this.gdv2.add("100", this.lp.address, 0, 15, [this.rewarder.address]);

    //     //deposit for bob
    //     await this.lp.connect(this.bob).approve(this.gdv2.address, "1000");
    //     await this.gdv2.connect(this.bob).deposit(0, "100");

    //     //deposit for alice
    //     await this.lp.connect(this.alice).approve(this.gdv2.address, "1000");
    //     await this.gdv2.connect(this.alice).deposit(0, "100");
    ...
    //     await this.gdv2.connect(this.bob).deposit(0, 0); //T+5
    ...
    it("should reward partner token accurately with 1 reward info after rewards run out", async function () {
      ...
      this.rewarder = await this.ComplexRewarderPerSec.deploy(
        this.partnerToken.address,
        this.gdv2.address,
        false
      );
      await this.rewarder.deployed();
      await this.partnerToken
        .connect(this.deployer)
        .approve(this.rewarder.address, "1000");

      await this.gdv2.add("100", this.lp.address, 0, 15, [
      ...
      await this.rewarder.addRewardInfo(
        0,
        starting.add(12),
      ...
      expect(await this.partnerToken.balanceOf(this.rewarder.address)).to.equal(
        400
      );

      await advanceTimeAndBlock(1); //Bob = T+1 <= ET = (40 / 2) * (ET - T+1) = (40 / 2) * (1) = 20
      await this.gdv2.connect(this.bob).deposit(0, 0); //Bob = T+2 <= ET = (40 / 2) * (ET - T+2) = (40 / 2) * (2) = 40
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(40);
      ...
      await this.gdv2.connect(this.bob).deposit(0, 0); //Bob = T+4 <= ET = (40 / 2) * (ET - T+2) = (40 / 2) * (4) = 80
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(
        40 + 80
      );
      ...
      expect(
        await this.rewarder.pendingTokens("0", this.alice.address)
      ).to.equal(120);
    });
  });
  ...
  //         this.chef = await this.MCV2.deploy(this.geos.address, this.dev.address, this.treasury.address, this.investor.address, this.geosPerSec, startTime, this.devPercent, this.treasuryPercent, this.investorPercent);
  //         await this.chef.deployed();

  //         this.rewarder = await this.SimpleRewarderPerBlock.deploy(this.geos.address, this.lp.address, this.partnerRewardPerBlock, this.chef.address);
  //         await this.rewarder.deployed();
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-56, b=17
  ...
  //         //   - 500 + 10*50 = 1000 (+50) GeosSwapTokenV2
  //         //   - 0 PartnerToken
  ...
  //         //   - 1000 + 10*50 = 1500 (+50) GeosSwapTokenV2
  ...
  //         //   - 450 + 10*50 = 950 (+50) GeosSwapTokenV2
  ...
  //         // Bob should have:
  //         //   - 1500 + 1*50 + 950 + 1*50 = 2550 (+50) GeosSwapTokenV2
  //         //   - 120 PartnerToken
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(2550, 2600);
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(120);
  //     });
  ...
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(500, 550);
  ...
  //         // At this point:
  //         //   Bob should have:
  //         //     - 500 + 5*50 = 750 (+50) GeosSwapTokenV2
  //         //     - 240 + 2*40 = 320 PartnerToken
  //         //   Dev should have: 15*20 = 300 (+20)
  //         //   Treasury should have: 15*20 = 300 (+20)
  //         //   Investor should have: 15*10 = 150 (+10)
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(750, 800);
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal("320");
  //         expect(await this.geos.balanceOf(this.dev.address)).to.be.within(300, 320);
  //         expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(300, 320);
  //         expect(await this.geos.balanceOf(this.investor.address)).to.be.within(150, 160);
  //         expect(await this.geos.totalSupply()).to.be.within(1500, 1600);
  //     });
  ...
  //         expect(await this.geos.totalSupply()).to.equal("0");
  ...
  //         expect(await this.geos.balanceOf(this.bob.address)).to.equal("0");
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal("0");
  //         expect(await this.geos.balanceOf(this.dev.address)).to.equal("0");
  ...
  //         // At this point:
  //         //   Bob should have:
  //         //     - 12*50 = 600 (+50) GeosSwapTokenV2
  //         //     - 3*40 = 120 PartnerToken
  //         //  Dev should have:
  //         //     - 12*20 = 240 (+20) GeosSwapTokenV2
  //         //  Treasury should have:
  //         //     - 12*20 = 240 (+20) GeosSwapTokenV2
  //         //  Investor should have:
  //         //     - 12*10 = 120 (+10) GeosSwapTokenV2
  //         expect(await this.geos.totalSupply()).to.be.within(1200, 1300);
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(600, 650);
  //         expect(await this.geos.balanceOf(this.dev.address)).to.be.within(240, 260);
  //         expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(240, 260);
  //         expect(await this.geos.balanceOf(this.investor.address)).to.be.within(120, 130);
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(120);
  //     });
  ...
  //         // Because LP rewards are divided among participants and rounded down, we account
  //         // for rounding errors with an offset
  //         expect(await this.geos.balanceOf(this.alice.address)).to.be.within(283 - this.tokenOffset, 333 + this.tokenOffset);
  ...
  //         expect(await this.geos.balanceOf(this.dev.address)).to.be.within(200 - this.tokenOffset, 220 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(200 - this.tokenOffset, 220 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.investor.address)).to.be.within(100 - this.tokenOffset, 110 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.chef.address)).to.be.within(217 - this.tokenOffset, 317 + this.tokenOffset);
  //         // Bob withdraws 5 LPs at t+30. At this point:
  //         //   Bob should have:
  //         //     - 4*50*2/3 + 2*50*2/6 + 10*50*2/7 = 309 (+50) GeosSwapTokenV2
  ...
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(309 - this.tokenOffset, 359 + this.tokenOffset);
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.be.within(101 - this.tokenOffset, 101 + this.tokenOffset);

  //         expect(await this.geos.balanceOf(this.carol.address)).to.equal("0");
  //         expect(await this.partnerToken.balanceOf(this.carol.address)).to.equal("0");

  //         expect(await this.geos.balanceOf(this.dev.address)).to.be.within(400 - this.tokenOffset, 420 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(400 - this.tokenOffset, 420 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.investor.address)).to.be.within(200 - this.tokenOffset, 210 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.chef.address)).to.be.within(408 - this.tokenOffset, 508 + this.tokenOffset);
  //         // Alice withdraws 20 LPs at t+40
  //         // Bob withdraws 15 LPs at t+50
  //         // Carol withdraws 30 LPs at t+60
  //         await advanceTimeAndBlock(9); // t+39, b=34
  ...
  //         //  - 283 + 10*50*2/7 + 10*50*20/65 = 579 (+50) GeosSwapTokenV2
  //         //  - 120 + 2*40*2/7 + 2*40*20/65 = 167 PartnerToken
  //         expect(await this.geos.balanceOf(this.alice.address)).to.be.within(579 - this.tokenOffset, 629 + this.tokenOffset);
  //         expect(await this.partnerToken.balanceOf(this.alice.address)).to.be.within(167 - this.tokenOffset, 167 + this.tokenOffset);
  //         // Bob should have:
  //         //  - 309 + 10*50*15/65 + 10*50*15/45 = 591 (+50) GeosSwapTokenV2
  //         //  - 102 + 2*40*15/65 + 2*40*15/45 = 147 PartnerToken
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(591 - this.tokenOffset, 641 + this.tokenOffset);
  //         expect(await this.partnerToken.balanceOf(this.bob.address)).to.be.within(147 - this.tokenOffset, 147 + this.tokenOffset);
  //         // Carol should have:
  //         //  - 2*50*3/6 + 10*50*3/7 + 10*50*30/65 + 10*50*30/45 + 10*50 = 1445 (+50) GeosSwapTokenV2
  //         //  - 2*40*1/2 + 2*40*3/7 + 2*40*30/65 + 2*40*30/45 + 2*40 = 244 PartnerToken
  //         expect(await this.geos.balanceOf(this.carol.address)).to.be.within(1328 - this.tokenOffset, 1378 + this.tokenOffset);
  //         expect(await this.partnerToken.balanceOf(this.carol.address)).to.be.within(244 - this.tokenOffset, 244 + this.tokenOffset);
  //         // Dev should have: 50*20 = 1000 (+20)
  //         // Treasury should have: 50*20 = 1000 (+20)
  //         // Investor should have: 50*10 = 500 (+10)
  //         expect(await this.geos.balanceOf(this.dev.address)).to.be.within(1000 - this.tokenOffset, 1020 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.treasury.address)).to.be.within(1000 - this.tokenOffset, 1020 + this.tokenOffset);
  //         expect(await this.geos.balanceOf(this.investor.address)).to.be.within(500 - this.tokenOffset, 510 + this.tokenOffset);
  //         // MasterChefJoe should have nothing
  //         expect(await this.geos.balanceOf(this.chef.address)).to.be.within(0, 0 + this.tokenOffset);
  ...
  //         //   - 100*50 = 5000 (+50) GeosSwapTokenV2
  ...
  //         //   - 5000 + 1*100*0.5 + 4*40*0.5 = 5130 (+50) GeosSwapTokenV2
  ...
  //         //   - 5130 + 1*40*0.5 + 20*40*0.5 = 5550 (+50) GeosSwapTokenV2
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-56
  ...
  //         // Carol should have:
  //         //  - 2*50*3/6 + 10*50*3/7 + 10*50*30/65 + 10*50*30/45 + 10*50 = 1328 (+50) GeosSwapTokenV2
  ...
  //         expect(await this.geos.balanceOf(this.alice.address)).to.be.within(682 - this.tokenOffset, 732 + this.tokenOffset);
  ...
  //         //   - 166 + 2*2/3*50 = 232 (+50) GeosSwapTokenV2
  //         //   - 0 PartnerToken
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(232 - this.tokenOffset, 282 + this.tokenOffset);
  //         expect(await this.rewarder.pendingTokens(this.bob.address)).to.equal(0);
  //     });
  ...
  //         await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address }); // t+10
  //         // At t+110, Alice should have:
  //         //   - 100*100*0.5 = 5000 (+50) GeosSwapTokenV2
  ...
  //         // At b=35, Alice should have:
  //         //   - 5130 + 21*40*0.5 = 5550 (+50) GeosSwapTokenV2
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-53, b=20
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-52, b=21
  ...
  //         // Bob should have:
  //         //   - 500 + 20*50 = 1500 (+50) GeosSwapTokenV2
  //         //   - 79 + 3*40*1/4 + 4*40*1/2 = 189 PartnerToken
  //         expect(await this.geos.balanceOf(this.bob.address)).to.be.within(1500, 1550);
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-53
  ...
  //         await this.geos.transferOwnership(this.chef.address); // t-52
